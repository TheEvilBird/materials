...
func (r *CmdRunner) Run(t *TC, opts RunOpts) (stop func(), err error) {
	...
	return func() {
		...
		if r.useDocker {
			cmd := exec.Command("docker", "ps", "--filter", "ancestor=hw3img", "--format", "{{.Names}}", "--latest")
			output, err := cmd.Output()
			if err != nil {
				Error(t, "Failed to run docker ps", zap.Error(err))
			}
			...
			if len(container) > 0 {
				...
				_, err := cmd.Output()
				if err != nil {
					Error(t, "Failed to run docker kill", zap.Error(err))
				}
			}
		}
	}, nil
