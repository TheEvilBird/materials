...
use anysystem::mc::strategy::{GoalFn, InvariantFn, PruneFn, StrategyConfig};
use anysystem::mc::{McState, ModelChecker};
        ...
        for name in &proc_names {
            ...
            for message in outbox {
                ...
                if message_data.contains(&message) {
                    return Err("Violated NO DUPLICATION".to_owned());
                }
                message_data.insert(message.clone());
                if message != sent_message {
                    return Err("Violated NO CREATION".to_owned());
                }
            }
        }
        Ok(())
