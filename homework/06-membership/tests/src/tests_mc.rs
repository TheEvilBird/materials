...
use anysystem::logger::{LogEntry, LogEntry::McMessageReceived};
use anysystem::mc::{
    predicates::{collects, goals, invariants, prunes},
    strategies::Bfs,
    CollectFn, EventOrderingMode, InvariantFn, McState, ModelChecker, StrategyConfig,
};
...
fn mc_explore_after_joins(sys: &mut System, seed_proc: String) -> Result<HashSet<McState>, String> {
    ...
    let strategy_config = StrategyConfig::default()
        ...
        // Time limit is set to 5 minutes, which should be more than enough.
        .invariant(invariants::time_limit(Duration::from_secs(300)))
    ...
    boxed!(move |state| {
        ...
        for node in state.node_states.keys() {
                ...
                .first()
            {
                ...
                if !members.eq(&group) {
                    return Err(format!(
                        "expected a stabilized group {group:?} but got {members:?}"
                    ));
                }
            }
        }
