...
use anysystem::mc::{
    predicates::{collects, goals, invariants},
    strategies::Bfs,
    InvariantFn, McState, ModelChecker, StrategyConfig,
};
...
pub fn test_mc_node_removed(config: &TestConfig) -> TestResult {
    ...
    achieved_states = Some(check_mc_node_removed(
        ...
        &removed_proc,
        15,
        achieved_states.unwrap_or_else(|| panic!("no states found after {keys_count} GET queries")),
    )?);
    let alive_proc = sys
        .process_names()
        .into_iter()
    ...
    S: Into<String>,
{
    ...
    let proc_name = proc.clone();
    let msg = Message::new("PUT", &format!(r#"{{"key": "{key}", "value": "{value}"}}"#));
