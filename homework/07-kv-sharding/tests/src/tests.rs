...
pub fn test_memory_overhead(config: &TestConfig) -> TestResult {
    ...
    assume!(
        !impl_code.contains("StorageNode.")
            && !impl_code.contains("self.__class__.")
            && !impl_code.contains("type(self)."),
        "Implementation uses class attributes, this is forbidden".to_string()
    )
}
...
pub fn test_node_added_during_inserts(config: &TestConfig) -> TestResult {
    let mut sys = build_system(config, false);
    let mut rand = Pcg64::seed_from_u64(config.seed);

    // send requests to insert random key-value pairs
    let keys_count = 100;
    let mut kv = HashMap::new();
    ...
    for _ in 0..keys_count {
        ...
        sys.send_local_message(
            &proc,
            Message::json("PUT", &PutReqMessage { key: &k, value: &v }),
        );
        *proc_calls.entry(proc).or_default() += 1;
        kv.insert(k, v);
    }

    // add new node to the system (while puts are processed)
    let added = sys.process_names().len().to_string();
    add_node(&added, &mut sys, config);
    send_node_added(&mut sys, &added);

    // wait for completion of puts
    for (proc, count) in proc_calls {
        wait_put_responses(&mut sys, &proc, count, 1000)?;
    }
...
fn check(
    ...
    if check_distribution {
        ...
        println!("  - average deviation from target: {avg_deviation:.3}");
        println!("  - max deviation from target: {max_deviation:.3}");
        assume!(
            max_deviation <= 0.1,
...
fn check_moved_keys(
    ...
    let mut total_count_before = 0;
    let mut total_count_after = 0;
    let mut not_moved_count = 0;
    let empty = HashSet::new();
    for proc in sys.process_names() {
        let b = before.get(&proc).unwrap_or(&empty);
        total_count_before += b.len() as u64;
        let a = after.get(&proc).unwrap_or(&empty);
        total_count_after += a.len() as u64;
    ...
    assume_eq!(
        ...
        total_count_before,
        format!(
            "Number of stored keys has changed (before: {}, after: {})",
            total_count_before, total_count_after
        ),
    )?;
    let moved_count = total_count_before - not_moved_count;
    let deviation = (moved_count as f64 - target as f64) / target as f64;
    println!("\nMoved keys:");
    println!("  - target: {target}");
    println!("  - observed: {moved_count}");
    println!("  - deviation: {deviation:.3}");
    assume!(
        deviation <= 0.1,
